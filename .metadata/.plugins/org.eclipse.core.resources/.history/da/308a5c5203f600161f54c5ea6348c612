...
public class WordCount {
	...
	public void countMainUFO() {
		...
		int count;
		for (int u = 0; u < main.MainUFO_Version_2_5.numUFO; u++) {
			if (main.MainUFO_Version_2_5.isUFOselected[u]){
				if (main.MainUFO_Version_2_5.phrases[u]==null) continue;
				//System.out.println("****"+main.MainUFO.phrases[u]+"*****");
				String[] p =  main.MainUFO_Version_2_5.phrases[u].split(",");
	...
	public void countSelectedList(ArrayList<Integer> a) {
		...
		for (int it = 0; it < a.size(); it++) {
			int index = a.get(it);	
			if (main.MainUFO_Version_2_5.phrases[index]==null) continue;
			String[] p =  main.MainUFO_Version_2_5.phrases[index].split(",");
			for (int i = 0; i < p.length; i++) {
				String word = p[i].toLowerCase();
				/*word = word.replace("?", "");
				word = word.replace(",", "");
	...
	public void countShape(ArrayList<Integer> a) {
		...
		for (int it = 0; it < a.size(); it++) {
			int index = a.get(it);	
			if (main.MainUFO_Version_2_5.shapeUFO[index]==null) continue;
			String word = main.MainUFO_Version_2_5.shapeUFO[index].toLowerCase();
			if (word.contains("((") || word.contains("( (") 
					|| word.contains("))") || word.contains(") )")
					|| word.contains("pd") || word.contains("nuforc")
					|| word.contains("!")){
